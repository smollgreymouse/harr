...
- Git repository state/history/branches/remotes/fetch/pull/push/commits: use `git-mcp` through `{{CTX_TOOLS}}`; pass the current repository/workspace path as `repo_path`. GitLab MR/pipeline/job/issue/project/server data: use `gitlab` through `{{CTX_TOOLS}}`.
