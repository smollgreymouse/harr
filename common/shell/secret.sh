...
migrate_gitlab_secret_from_leanctx() {
  ...
  [[ -n "$target" && ! -s "$target" && -r "$HARR_LEANCTX_CONFIG" ]] || return 0
...
cmd_secret_set() {
  ...
  if [[ ! -t 0 ]]; then IFS= read -r value || true; else read -rsp "${prompt:-$name}: " value; printf '\n'; fi
...
cmd_secret() {
  ...
  shift || true
  case "$command" in
    set) [[ $# -eq 1 ]] || die 'usage: harr secret set NAME'; cmd_secret_set "$1" ;;
    status) cmd_secret_status "$@" ;;
    unset) [[ $# -eq 1 ]] || die 'usage: harr secret unset NAME'; cmd_secret_unset "$1" ;;
