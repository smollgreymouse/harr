...
- If a GitLab push fails because SSH credentials are unavailable to the current host or sandbox (for example `Permission denied (publickey)`), retry the same real Git push through Harr's host-independent secure HTTPS bridge: `harr git push [git-push-options] [remote] [refspec...]` via `{{CTX_SHELL}}`. Harr rewrites only that process's GitLab transport to HTTPS, reads the stored GitLab PAT through `GIT_ASKPASS`, never embeds the PAT in argv/remote/history, and preserves the real local commit SHA and full Git semantics including deletions, binary files, symlinks and mode changes.
- Only if both normal Git transport and `harr git push` are unavailable may repository-file API mirroring be considered. If the local diff can be represented exactly through GitLab MCP, create the remote branch from the intended target, mirror the local diff (prefer a batched/single-commit file API when available), verify the resulting remote branch diff against the intended local diff, then create the MR. Do not use browser/curl/manual tokens while Harr's GitLab transport or enabled GitLab MCP can perform the operation. If the diff cannot be represented faithfully, report that limitation instead of approximating it.
