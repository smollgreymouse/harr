...
When GitLab is enabled, `harr gitlab fetch`, `harr gitlab publish`, and `harr gitlab push` use Harr's HTTPS/PAT transport. `harr gitlab publish` is the branch-safe host-independent MR source publisher: it uses the current local branch name, pushes `HEAD` to the same-named remote branch, verifies the remote SHA, and normalizes upstream to that branch. `harr gitlab push` is the lower-level command for custom push options/refspecs. These commands never require SSH credentials, change a repository remote URL, or write global Git URL rewrites.
