...
managed_mcp_names() { require_mcp_manager; python3 "$HARR_MCP_MANAGER" names --lifecycle service; }
all_mcp_names() { require_mcp_manager; python3 "$HARR_MCP_MANAGER" names; }
mcp_exists() { managed_mcp_names | grep -Fxq -- "$1"; }
require_mcp() { mcp_exists "$1" || die "unknown or non-service Harr MCP: $1"; }
mcp_env_file() { printf '%s/%s.env\n' "$HARR_MCP_CONFIG_DIR" "$1"; }
mcp_server_field() { require_mcp_manager; python3 "$HARR_MCP_MANAGER" server-field "$1" "$2"; }
env_value() {
