...
class TaskPage(QWidget):
    def __init__(
        ...
        self.prompt = QTextEdit(); self.prompt.setPlaceholderText("What should Codex do?")
        ...
        self.primary_button = QPushButton("Schedule")
        ...
        transcript = QWidget(); self.transcript_layout = QVBoxLayout(transcript)
    ...
    def _load_task(self, task: dict) -> None:
        model_label = next((key for key, value in MODELS.items() if value == task.get("model")), "Terra")
        reasoning_label = next((key for key, value in REASONING.items() if value == task.get("reasoning")), "High")
        speed_label = next((key for key, value in SPEEDS.items() if value == task.get("speed")), "Standard")
        self.model.setCurrentText(model_label)
        self.reasoning.setCurrentText(reasoning_label)
        self.speed.setCurrentText(speed_label)
        self.session.setText(str(task.get("session") or ""))
    ...
    def _connect_draft_signals(self) -> None:
        ...
        self.session.textChanged.connect(self._session_changed)
