...
class MainWindow(QMainWindow):
    def __init__(self, store: TaskStore | None = None) -> None:
        ...
        self.splitter.setSizes([self._sidebar_width, 850])
    ...
    def _build_sidebar(self) -> None:
        ...
        self.task_tree.setIndentation(14)
    ...
    def _build_workspace(self) -> None:
        ...
        tab_bar = PlusTabBar(self.tabs)
        tab_bar.plusClicked.connect(self.new_task)
        ...
        self.sidebar_toggle.toggled.connect(self.set_sidebar_visible)
        self.tabs.setCornerWidget(self.sidebar_toggle, Qt.Corner.TopLeftCorner)

        self.empty_hint = QLabel("Create a task with + or reopen one from the tasks sidebar", host)
    ...
    def set_sidebar_visible(self, visible: bool) -> None:
        if visible:
            self.sidebar.show()
            self.splitter.setSizes([self._sidebar_width, max(400, self.width() - self._sidebar_width)])
        else:
            sizes = self.splitter.sizes()
            if sizes and sizes[0] > 0:
                self._sidebar_width = max(215, min(390, sizes[0]))
            self.sidebar.hide()
    ...
    def open_task(self, task_id: str, *, make_current: bool = True) -> None:
        ...
        set_tab_close_button(self.tabs.tabBar(), index, lambda tid=task_id: self.close_task_by_user(tid))
    ...
    def close_task_by_user(self, task_id: str) -> None:
    ...
    def _sidebar_group_state(self) -> dict[str, bool]:
        state = {"active": True, "history": False}
    ...
    def refresh_sidebar(self) -> None:
        expansion = self._sidebar_group_state() if self.task_tree.topLevelItemCount() else {"active": True, "history": False}
        query = self.search.text().strip().lower() if hasattr(self, "search") else ""
        tasks = self.store.list()
        self.task_tree.blockSignals(True)
        self.task_tree.clear()
        active_root = self._add_group("active", "ACTIVE")
        history_root = self._add_group("history", "HISTORY")
        active_count = 0
        history_count = 0
        for task in tasks:
            haystack = " ".join([
        ...
        active_root.setExpanded(expansion.get("active", True))
        history_root.setExpanded(expansion.get("history", False))
        self.task_tree.blockSignals(False)
        self.sidebar_status.setText(f"{len(tasks)} task{'s' if len(tasks) != 1 else ''}")

    def _add_group(self, key: str, text: str) -> QTreeWidgetItem:
    ...
    def rebuild_tasks_menu(self) -> None:
        ...
        refresh_sessions = self.tasks_menu.addAction("↻ Refresh Codex sessions")
        refresh_sessions.triggered.connect(self.refresh_codex_sessions)
        ...
        if not history:
            empty = history_menu.addAction("No finished tasks")
            empty.setEnabled(False)
        for task in history:
