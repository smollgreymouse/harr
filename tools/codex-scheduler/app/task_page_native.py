...
def task_display_title(task: dict, limit: int = 42) -> str:
    ...
    if session:
        return session if len(session) <= 18 else session[:8] + "…" + session[-6:]
    return "New task"
...
class SessionComboBox(QComboBox):
    """Recent-session dropdown that still accepts arbitrary pasted IDs."""
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        line.setPlaceholderText("Session ID")
    ...
    def _session_activated(self, index: int) -> None:
        ...
        if isinstance(session_id, str) and session_id:
            ...
            line = self.lineEdit()
            if line is not None:
                line.setCursorPosition(len(session_id))
    ...
    def set_sessions(self, sessions: list[dict[str, Any]], *, select_latest_if_empty: bool = False) -> None:
        ...
        for session in sessions:
            ...
            if not session_id:
                continue
            preview = " ".join(str(session.get("name") or session.get("preview") or "Codex session").split())
            if len(preview) > 42:
                preview = preview[:41] + "…"
...
class TaskPage(QWidget):
    def __init__(self, store: TaskStore, task: dict, on_changed: Callable[[str], None], parent: QWidget | None = None) -> None:
        ...
        self.session = SessionComboBox(self)
        self.session.setToolTip("Recent Codex sessions; paste any session ID with Ctrl+V")
        self.when = QPushButton("Run at", self)
        self.when.setToolTip("Choose date and time")
        self.when.clicked.connect(self.choose_run_time)
        ...
        self.project_button = QToolButton(self); self.project_button.setText("⋮"); self.project_button.setToolTip("Session project")
        ...
        self.prompt = QTextEdit(self); self.prompt.setPlaceholderText("What should Codex do?"); self.prompt.setMinimumHeight(100)
        root.addWidget(self.prompt)

        actions = QHBoxLayout()
        self.save_button = QPushButton("Save final answer…", self); self.save_button.setCheckable(True); self.save_button.clicked.connect(self.toggle_save_answer)
        self.primary_button = QPushButton("Schedule", self); self.primary_button.clicked.connect(self.primary_action)
    ...
    def _load_task(self, task: dict) -> None:
        self.model.setCurrentText(next((k for k, v in MODELS.items() if v == task.get("model")), "Terra"))
        self.reasoning.setCurrentText(next((k for k, v in REASONING.items() if v == task.get("reasoning")), "High"))
        self.speed.setCurrentText(next((k for k, v in SPEEDS.items() if v == task.get("speed")), "Standard"))
        ...
        if task.get("scheduled"):
            ...
            self._scheduled_time = default_run_time()
        self._refresh_when_button()
    ...
    def _connect_draft_signals(self) -> None:
        ...
        line = self.session.lineEdit()
        assert line is not None
        line.editingFinished.connect(self.resolve_cwd_preview)
    ...
    def persist_draft(self) -> None:
        ...
        self.store.patch(
            self.task_id,
            model=MODELS[self.model.currentText()],
            reasoning=REASONING[self.reasoning.currentText()],
            speed=SPEEDS[self.speed.currentText()],
    ...
    def choose_run_time(self) -> None:
        if self.current_task().get("status") != "draft":
            return
        initial = self._scheduled_time or default_run_time()
        dialog = ScheduleTimeDialog(initial, self)
        if dialog.exec() != dialog.DialogCode.Accepted:
            return
        self._scheduled_time = dialog.selected_date_time()
        self._refresh_when_button()
        self.persist_draft()

    def _refresh_when_button(self) -> None:
        if self._scheduled_time is None:
            self.when.setText("Run at")
            return
        self.when.setText(self._scheduled_time.toString("dd.MM.yyyy  HH:mm"))
    ...
    def resolve_cwd_preview(self) -> None:
        ...
        try:
            ...
            self.invalidate_cwd_preview()
            self.status.setText(str(exc)[:180])
            return
        self.set_resolved_cwd(cwd)
        self.persist_draft()
        self.status.setText("✓ Project verified")
    ...
    def set_save_button_state(self, path: Path | None) -> None:
        ...
        if path is None:
            ...
            self.save_button.setToolTip("Choose a file and enable answer export")
        else:
            self.save_button.setText(f"✓ {path.name}")
            self.save_button.setToolTip(str(path))

    def toggle_save_answer(self, checked: bool) -> None:
        if self.current_task().get("status") != "draft":
            self.save_button.setChecked(self._answer_path is not None)
            return
        if not checked:
    ...
    def primary_action(self) -> None:
        ...
        if status == "draft":
            self.schedule()
        elif status in {"scheduled", "running", "cancelling"}:
            self.cancel_task()
    ...
    def schedule(self) -> None:
        ...
        if not session or not prompt:
            QMessageBox.warning(self, APP_NAME, "Session ID and prompt are required.")
            return
        if self._scheduled_time is None:
            QMessageBox.warning(self, APP_NAME, "Choose a run time first.")
            return
        try:
            cwd = resolve_session_cwd(session)
        except SessionResolutionError as exc:
        ...
        if self._answer_path:
            args += ["--save-answer", str(self._answer_path)]
        proc = subprocess.run(args, text=True, capture_output=True)
        combined = (proc.stdout + proc.stderr).strip()
        if proc.returncode != 0:
            QMessageBox.critical(self, APP_NAME, combined or f"Scheduler failed with exit code {proc.returncode}")
            return
        match = re.search(r"\bjob\s+(\d+)\b", combined)
        task = self.store.patch(
            self.task_id,
            status="scheduled",
            ...
            session=session,
            prompt=prompt,
            scheduled=self._scheduled_time.toString(Qt.DateFormat.ISODate),
            cwd=str(cwd),
            log=str(log_path),
            answer=str(self._answer_path) if self._answer_path else None,
    ...
    def cancel_task(self) -> None:
        task = self.current_task()
        if task.get("status") not in {"scheduled", "running", "cancelling"}:
            return
        if QMessageBox.question(
            self,
            APP_NAME,
            "Cancel this Codex task?",
    ...
    def add_bubble(self, role: str, text: str) -> MessageBubble:
        ...
        self.transcript_layout.addWidget(bubble)
    ...
    def _rebuild_transcript(self, task: dict) -> None:
        ...
        if task.get("status") != "draft" and prompt:
            self.add_bubble("user", prompt)
        log_path = Path(str(task.get("log"))) if task.get("log") else None
        self._log_path = log_path
    ...
    def handle_event(self, event: TranscriptEvent) -> None:
        if event.kind == "assistant":
            if self._assistant_bubble is None:
                self._assistant_bubble = self.add_bubble("assistant", event.text)
            else:
                self._assistant_bubble.append_text(event.text)
        elif event.kind == "error":
            self.status.setText("! " + event.text[:160])
    ...
    def _apply_status(self, task: dict) -> None:
        ...
        self.status.setText(task_status_text(task))
        editable = status == "draft"
        for widget in (self.model, self.reasoning, self.speed, self.session, self.prompt, self.when, self.save_button):
            widget.setEnabled(editable)
        self.project_button.setEnabled(True)
        if status == "draft":
            self.primary_button.setText("Schedule")
            self.primary_button.setEnabled(True)
        elif status in {"scheduled", "running"}:
            self.primary_button.setText("Cancel task")
            self.primary_button.setEnabled(True)
        elif status == "cancelling":
            self.primary_button.setText("Cancelling…")
            self.primary_button.setEnabled(False)
        else:
            self.primary_button.setText(STATUS_TEXT.get(status, status.title()))
            self.primary_button.setEnabled(False)
