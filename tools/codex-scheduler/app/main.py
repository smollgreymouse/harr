...
class MainWindow(QMainWindow):
    def __init__(self, store: TaskStore | None = None) -> None:
        ...
        self.session_refresh_timer = QTimer(self)
        self.session_refresh_timer.setInterval(30000)
        self.session_refresh_timer.timeout.connect(self.refresh_codex_sessions)
        ...
        QTimer.singleShot(0, self.refresh_codex_sessions)
    ...
    def setup_tray(self) -> None:
        ...
        menu.addAction(show_action); menu.addAction(new_action); menu.addSeparator(); menu.addAction(quit_action)
    ...
    def restore_workspace(self) -> None:
        ...
        for task_id in ui.get("open_task_ids", []):
            if self.store.load(str(task_id)) is not None:
                self.open_task(str(task_id), make_current=False); opened = True
        current = ui.get("current_task_id")
        if current and str(current) in self._page_by_id:
            self.tabs.setCurrentWidget(self._page_by_id[str(current)])
        if not opened and not self.store.list(): self.new_task()
        self.update_empty_hint()

    def refresh_codex_sessions(self) -> None:
        try:
            sessions = list_codex_sessions(timeout=5.0, limit=50)
        except SessionResolutionError as exc:
            if not self._session_choices:
                self.sidebar_status.setToolTip(f"Could not load Codex sessions: {exc}")
            return
        self._session_choices = sessions
        self.sidebar_status.setToolTip(f"{len(sessions)} recent Codex sessions loaded")
        for page in list(self._page_by_id.values()):
    ...
    def open_task(self, task_id: str, *, make_current: bool = True) -> None:
        if task_id in self._page_by_id:
            if make_current: self.tabs.setCurrentWidget(self._page_by_id[task_id])
            return
        ...
        if self._session_choices:
            page.set_session_choices(self._session_choices, select_latest_if_empty=True)
        if make_current: self.tabs.setCurrentIndex(index)
        self.update_empty_hint(); self.save_ui_state()
    ...
    def tab_tooltip(self, task: dict) -> str:
        ...
        if task.get("session"): bits.append(str(task["session"]))
        if task.get("scheduled"): bits.append(str(task["scheduled"]))
        return "\n".join(bits)

    def close_tab(self, index: int) -> None:
        page = self.tabs.widget(index)
        if not isinstance(page, TaskPage): self.tabs.removeTab(index); return
        task_id = page.task_id; task = self.store.load(task_id)
        self.tabs.removeTab(index); self._page_by_id.pop(task_id, None); page.deleteLater()
        if task is not None and self.store.is_empty_draft(task): self.store.delete(task_id, delete_log=True, delete_answer=False)
        self.save_ui_state(); self.refresh_sidebar(); self.update_empty_hint()
    ...
    def close_task_tab_by_id(self, task_id: str, *, preserve_task: bool = True) -> None:
        ...
        if index < 0: return
        if preserve_task:
            self.tabs.removeTab(index); self._page_by_id.pop(task_id, None); page.deleteLater(); self.save_ui_state(); self.update_empty_hint()
        else: self.close_tab(index)
    ...
    def task_changed(self, task_id: str) -> None:
        ...
        if page is not None:
            index = self.tabs.indexOf(page)
            if index >= 0:
                self.tabs.setTabText(index, self.tab_title(task)); self.tabs.setTabToolTip(index, self.tab_tooltip(task))
        self.refresh_sidebar()
    ...
    def refresh_all(self) -> None:
        ...
        for task_id, page in list(self._page_by_id.items()):
            task = self.store.load(task_id)
            if task is None:
                self.close_task_tab_by_id(task_id, preserve_task=True); continue
            index = self.tabs.indexOf(page)
            if index >= 0: self.tabs.setTabText(index, self.tab_title(task))

    def refresh_sidebar(self) -> None:
        query = self.search.text().strip().lower() if hasattr(self, "search") else ""
        tasks = self.store.list()
        self.task_tree.blockSignals(True); self.task_tree.clear()
        active_root = self._add_group("ACTIVE"); history_root = self._add_group("HISTORY")
        active_count = history_count = 0
        for task in tasks:
            haystack = " ".join([str(task.get("session") or ""), str(task.get("prompt") or ""), str(task.get("cwd") or ""), str(task.get("model") or ""), str(task.get("status") or "")]).lower()
            if query and query not in haystack: continue
            status = str(task.get("status") or "draft")
            root = active_root if status in ACTIVE_STATUSES else history_root
            if root is active_root: active_count += 1
            else: history_count += 1
            self._add_task_item(root, task)
        active_root.setText(0, f"ACTIVE  {active_count}"); history_root.setText(0, f"HISTORY  {history_count}")
        active_root.setExpanded(True); history_root.setExpanded(True); self.task_tree.blockSignals(False)
        self.sidebar_status.setText(f"{len(tasks)} task{'s' if len(tasks) != 1 else ''}")

    def _add_group(self, text: str) -> QTreeWidgetItem:
        item = QTreeWidgetItem([text]); item.setFlags(Qt.ItemFlag.ItemIsEnabled)
        font = item.font(0); font.setBold(True); item.setFont(0, font); self.task_tree.addTopLevelItem(item); return item

    def _add_task_item(self, root: QTreeWidgetItem, task: dict) -> None:
        title = task_display_title(task, 34); status = str(task.get("status") or "draft")
        model = str(task.get("model") or "").removeprefix("gpt-5.6-").title()
        when = str(task.get("scheduled") or ""); when = when.replace("T", " ")[:16] if when else "unscheduled"
        item = QTreeWidgetItem([f"{STATUS_ICON.get(status, '•')}  {title}\n    {model} · {when}"])
        item.setData(0, TASK_ID_ROLE, str(task["id"])); item.setToolTip(0, self.tab_tooltip(task)); root.addChild(item)

    def tree_item_clicked(self, item: QTreeWidgetItem, _column: int) -> None:
        task_id = item.data(0, TASK_ID_ROLE)
        if task_id: self.open_task(str(task_id))
    ...
    def open_task_context_menu(self, pos: QPoint) -> None:
        ...
        if task is None: return
        menu = QMenu(self); self.populate_task_actions(menu, task); menu.exec(self.task_tree.viewport().mapToGlobal(pos))

    def rebuild_tasks_menu(self) -> None:
        self.tasks_menu.clear()
        new_action = self.tasks_menu.addAction("＋ New task"); new_action.triggered.connect(self.new_task)
        refresh_sessions = self.tasks_menu.addAction("↻ Refresh Codex sessions"); refresh_sessions.triggered.connect(self.refresh_codex_sessions)
        self.tasks_menu.addSeparator()
        active_menu = self.tasks_menu.addMenu("Active"); history_menu = self.tasks_menu.addMenu("History")
        tasks = self.store.list(); active = [task for task in tasks if task.get("status") in ACTIVE_STATUSES]; history = [task for task in tasks if task.get("status") in FINISHED_STATUSES]
        if not active: empty = active_menu.addAction("No active tasks"); empty.setEnabled(False)
        for task in active: sub = active_menu.addMenu(self.tab_title(task)); self.populate_task_actions(sub, task)
        if not history: empty = history_menu.addAction("No finished tasks"); empty.setEnabled(False)
        for task in history: sub = history_menu.addMenu(self.tab_title(task)); self.populate_task_actions(sub, task)
        if any(task.get("status") in {"scheduled", "running", "cancelling"} for task in active):
            self.tasks_menu.addSeparator(); cancel_all = self.tasks_menu.addAction("Cancel all active jobs…"); cancel_all.triggered.connect(self.cancel_all_active)

    def populate_task_actions(self, menu: QMenu, task: dict) -> None:
        task_id = str(task["id"])
        open_action = menu.addAction("Open"); open_action.triggered.connect(lambda _checked=False, tid=task_id: self.open_task(tid))
        if task_id in self._page_by_id:
            close_action = menu.addAction("Close tab"); close_action.triggered.connect(lambda _checked=False, tid=task_id: self.close_task_tab_by_id(tid, preserve_task=True))
        status = str(task.get("status") or "draft")
        if status in {"scheduled", "running", "cancelling"}:
            menu.addSeparator(); cancel = menu.addAction("Cancel task…"); cancel.triggered.connect(lambda _checked=False, tid=task_id: self.cancel_task(tid)); return
        menu.addSeparator()
        delete_data = menu.addAction("Delete task data…"); delete_data.setToolTip("Delete scheduler metadata and internal JSONL log; keep exported answer")
        delete_data.triggered.connect(lambda _checked=False, tid=task_id: self.delete_task(tid, delete_answer=False))
        if task.get("answer"):
            delete_all = menu.addAction("Delete task + saved answer…"); delete_all.triggered.connect(lambda _checked=False, tid=task_id: self.delete_task(tid, delete_answer=True))

    def cancel_task(self, task_id: str) -> None:
        task = self.store.load(task_id)
        if task is None or task.get("status") not in {"scheduled", "running", "cancelling"}: return
        if QMessageBox.question(self, APP_NAME, f"Cancel task “{task_display_title(task, 52)}”?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) != QMessageBox.StandardButton.Yes: return
        try: self.store.cancel(task_id)
        except Exception as exc: QMessageBox.critical(self, APP_NAME, str(exc)); return
        page = self._page_by_id.get(task_id)
        if page is not None: page.refresh_from_store()
        self.refresh_sidebar()
    ...
    def cancel_all_active(self) -> None:
        ...
        if not jobs: return
        if QMessageBox.question(self, APP_NAME, f"Cancel {len(jobs)} active Codex task(s)?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) != QMessageBox.StandardButton.Yes: return
        errors: list[str] = []
        for task in jobs:
            try: self.store.cancel(str(task["id"]))
            except Exception as exc: errors.append(f"{task_display_title(task)}: {exc}")
        self.refresh_all()
        if errors: QMessageBox.warning(self, APP_NAME, "\n".join(errors))

    def delete_task(self, task_id: str, *, delete_answer: bool) -> None:
        task = self.store.load(task_id)
        if task is None: return
        if task.get("status") in {"scheduled", "running", "cancelling"}:
            QMessageBox.warning(self, APP_NAME, "Cancel the active task before deleting it."); return
        ...
        if QMessageBox.question(self, APP_NAME, f"Delete scheduler data for “{task_display_title(task, 52)}”?{extra}", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No) != QMessageBox.StandardButton.Yes: return
        self.close_task_tab_by_id(task_id, preserve_task=True)
        try: self.store.delete(task_id, delete_log=True, delete_answer=delete_answer)
        except Exception as exc: QMessageBox.critical(self, APP_NAME, str(exc)); return
        self.refresh_sidebar(); self.save_ui_state()

    def save_ui_state(self) -> None:
        ids: list[str] = []
        for index in range(self.tabs.count()):
            page = self.tabs.widget(index)
            if isinstance(page, TaskPage): ids.append(page.task_id)
        current = self.tabs.currentWidget(); current_id = current.task_id if isinstance(current, TaskPage) else None
        self.store.save_ui(ids, current_id)

    def update_empty_hint(self) -> None:
        empty = self.tabs.count() == 0; self.tabs.setVisible(not empty); self.empty_hint.setVisible(empty)
    ...
    def quit_app(self) -> None:
        self._quitting = True; self.save_ui_state(); self.tray.hide(); QApplication.instance().quit()
